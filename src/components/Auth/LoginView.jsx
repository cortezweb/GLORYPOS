...
export default function LoginView({ onGoToRegister }) {
  ...
  // Inicializar con el primer usuario de la lista
  useEffect(() => {
  ...
  const registerFailedAttempt = () => {
    ...
    if (next >= 5) {
      const until = Date.now() + 30_000; // 30 segundos de bloqueo
  ...
  // Demo rápido: usa loginWithPin que ya hashea internamente
  const handleFastDemoLogin = async (usr) => {
    if (isLocked) return;
    setSelectedUser(usr);
    setIsLoggingIn(true);
    // El PIN demo visible al usuario (texto plano) — loginWithPin lo hashea internamente
              ...
              {usuarios.map((usr) => {
                ...
                return (
                  <button
                    ...
                    disabled={isLocked}
                    className="px-2.5 py-1.5 rounded-xl bg-slate-100 hover:bg-blue-50 text-slate-700 hover:text-blue-700 font-bold border border-slate-200 flex items-center justify-center gap-1 transition disabled:opacity-40 disabled:cursor-not-allowed"
                    title={`PIN: ${rawPin}`}
                  >
                    <span>{usr.rol === 'ADMIN' ? '👑' : usr.rol === 'CAJERO' ? '🛒' : '✨'}</span>
      ...
      <footer className="max-w-md mx-auto w-full text-center space-y-2 pb-2">
        ...
        {onGoToRegister && (
          <button
            ...
            className="inline-flex items-center gap-1.5 text-[11px] font-bold text-indigo-300 hover:text-white transition"
          >
            <UserPlus className="w-3.5 h-3.5" />
            ¿Eres nuevo? Registra tu negocio →
          </button>
        )}
        <div className="flex items-center justify-center gap-3 text-[11px] text-slate-400">
          <span className="flex items-center gap-1">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
            <span>Contraseñas SHA-256</span>
          </span>
          <span>•</span>
